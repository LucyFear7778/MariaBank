...
# 2. DATABASE REBUILD
echo "[*] Building Database with 30 Staff and 50 Clients..."
sqlite3 bank.db <<SQL
...
# 4. DASHBOARD (FIXED IDOR & COMPLETE DATA DISPLAY)
cat << 'PHP' > dash.php
...
<div class="main">
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
        <div class="card"><h3>💸 Money Transfer (IDOR)</h3>
            <form method="POST"><input name="f" placeholder="Sender Account ID" style="width:100%; padding:8px; margin:5px 0;">
            <input name="t" placeholder="Receiver Account ID" style="width:100%; padding:8px; margin:5px 0;">
            <input name="a" placeholder="Amount (₱)" style="width:100%; padding:8px; margin:5px 0;">
            <button type="submit" class="btn">TRANSFER NOW</button></form>
        ...
        <div class="card"><h3>💻 RCE Terminal</h3><form><input name="c" placeholder="sh command..." style="width:100%; padding:8px;"></form>
            <div class="terminal"><pre><?php if(isset($_GET['c'])) { echo shell_exec($_GET['c']); } ?></pre></div>
        </div>
    </div>
    <div class="card"><h3>🌍 Rich Clients (50 Targets)</h3><div style="max-height:200px; overflow-y:auto;"><table>
    ...
    <div class="card"><h3>👔 Bank Staff (30 Employees)</h3><div style="max-height:200px; overflow-y:auto;"><table>
...
# 5. DEFACEMENT PAGE
cat << 'PHP' > hacked.php
<!DOCTYPE html><html><head><title>HACKED BY SADFRIENDS</title>
<style>body{background:black; color:#00ff00; text-align:center; font-family:monospace; padding-top:100px;}
h1{font-size:60px; text-shadow: 0 0 10px #ff0000;} .msg{color:white; font-size:25px;}</style></head>
<body><h1>SYSTEM PWNED</h1><div class="msg">SADMARIA BANK SECURITY BYPASSED</div><br>
<img src="https://media.giphy.com" width="400"><br>
<p style="font-size:20px; color:red;">Greets to: SadFriends | LucyFear7778</p>
<p style="color:white;">Your security is our playground.</p></body></html>
PHP
...
echo -e "\e[1;33m[!] Defacement Page: http://localhost:8080/hacked.php\e[0m"
