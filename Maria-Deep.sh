...
# 1. SETUP DIRECTORIES (Fixed Order)
rm -rf ~/maria_banko
mkdir -p ~/maria_banko/login
cd ~/maria_banko
pkg install php sqlite -y > /dev/null 2>&1

# 2. DATABASE INITIALIZATION
echo "[*] Initializing Database Tables..."
sqlite3 bank.db "CREATE TABLE users (id INTEGER PRIMARY KEY, username TEXT, password TEXT, role TEXT); CREATE TABLE employees (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, position TEXT, salary REAL); CREATE TABLE customers (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, balance REAL, pin TEXT); INSERT INTO users (username, password, role) VALUES ('admin', '0192023a7bbd73250516f069df18b500', 'admin');"

# 3. DATA SEEDING (30 Staff & 50 Millionaire Clients)
echo "[*] Seeding 30 Staff & 50 Clients..."
...
# 5. DASHBOARD (Fixed Transfer logic & Defacement)
cat << 'PHP' > dash.php
<?php 
if(!isset($_COOKIE['auth'])) { header("Location: login/"); exit(); }
$db = new SQLite3('bank.db'); $msg = "";
...
<div class="main">
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
        <div class="card"><h3>💸 Transfer Millions (IDOR)</h3>
            <form method="POST"><input name="f" placeholder="Sender ID" style="width:100%; padding:8px; margin:5px 0;"><input name="t" placeholder="Receiver ID" style="width:100%; padding:8px; margin:5px 0;"><input name="a" placeholder="Amount (₱)" style="width:100%; padding:8px; margin:5px 0;"><button type="submit" class="btn">TRANSFER</button></form>
            <p style="color:green; font-weight:bold;"><?php echo $msg; ?></p>
        </div>
        <div class="card"><h3>💻 Terminal (Type 'deface')</h3>
            <form><input name="c" placeholder="sh command..." style="width:100%; padding:8px;"></form>
            <div class="terminal"><pre><?php 
                if(isset($_GET['c'])) {
                    if($_GET['c'] == 'deface') {
                        file_put_contents('index.php', '<html><body style="background:black;color:red;text-align:center;padding-top:100px;font-family:monospace"><h1>HACKED BY SADFRIENDS</h1><p>SYSTEM PWNED</p></body></html>');
                        echo "DONE! Go to main site.";
...
<form method="POST"><input name="u" placeholder="User" style="width:100%; padding:10px; margin:5px 0;"><input type="password" name="p" placeholder="Pass" style="width:100%; padding:10px; margin:5px 0;"><button class="btn">LOGIN</button></form>
</div></body></html>
PHP
