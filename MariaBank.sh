...
echo -e "\e[1;34m[*] DEPLOYING MARIABANK V36: THE ULTIMATE PENTEST LAB...\e} $i"
    JOB="${p_jobs[$((i % 5))]}"
...
# 3. ORIGINAL CSS (White/Blue Theme)
cat << 'CSS' > style.css
...
.footer { text-align: center; color: #001f3f; font-size: 10px; font-weight: bold; margin-top: 20px; }
CSS
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    else { $err = "Login failed! SadFriends Protection."; }
} ?>
...
<div class="main">
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
        <div class="card"><h3>💸 Money Transfer (IDOR)</h3>
            <form method="POST"><input name="f" placeholder="Sender Account ID"><input name="t" placeholder="Receiver Account ID"><input name="a" placeholder="Amount"><button class="btn">TRANSFER</button></form>
            <?php if($_SERVER['REQUEST_METHOD']=='POST'){ $f=$_POST['f']; $t=$_POST['t']; $a=$_POST['a'];
                ...
                echo "<p style='color:green; font-size:12px;'>Transferred successfully!</p>"; } ?>
        </div>
    ...
    <div class="card"><h3>👔 Bank Staff (Pinoy)</h3><table><tr><th>Name</th><th>Position</th></tr>
    <?php $r=$db->query("SELECT * FROM employees"); while($row=$r->fetchArray()){ echo "<tr><td>{$row['name']}</td><td><b>{$row['position']}</b></td></tr>"; } ?>
    </table></div>
    <div class="card"><h3>🌍 Rich Clients (Targets)</h3><table><tr><th>ID</th><th>Name</th><th>Balance</th></tr>
    ...
    <div class="card" style="border-top: 5px solid red;"><h3>🔓 LEAKED CREDENTIALS INDEX (MD5)</h3><table><tr><th>User</th><th>Plain</th><th>Hash</th></tr>
    <?php $r=$db->query("SELECT * FROM users"); while($row=$r->fetchArray()){ $p = ($row['role']=='admin') ? 'admin123' : 'password123';
        echo "<tr><td><code>{$row['username']}</code></td><td><b style='color:blue;'>$p</b></td><td><small>{$row['password']}</small></td></tr>"; } ?>
    </table></div>
    <div class="footer">BY SADFRIENDS</div>
</div></body></html>
PHP
